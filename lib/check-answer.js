...
function parseAnswerline(answerline) {
    ...
    let mainAnswers = mainAnswer.split(' or ').map(token => token.trim()).filter(token => token.length > 0);

    const parsedAnswerline = { accept: [], prompt: [], reject: [] };

    mainAnswers.forEach(answer => addAnswer(parsedAnswerline, answer, 'accept'));

    addAbbreviationAnswers(parsedAnswerline, mainAnswer, 'accept');
    addSpecialCaseAnswers(parsedAnswerline, answerline, 'accept', /[[(]accept either/i, /[[(]accept any/i);
    addSpecialCaseAnswers(parsedAnswerline, answerline, 'prompt', /prompt on (a )?partial/i);

    const clauses = splitAnswerlineIntoClauses(subAnswer);
    clauses.forEach(clause => processClause(clause, parsedAnswerline));

    return parsedAnswerline;
}

function addAnswer(parsedAnswerline, answer, type) {
    ['extractUnderlining', 'extractKeyWords', 'extractQuotes'].forEach(extractionMethod => {
        const extractedAnswer = utils[extractionMethod](answer);
        if (type !== 'prompt') {
            parsedAnswerline[type].push(extractedAnswer);
        } else {
            parsedAnswerline[type].push([extractedAnswer, null]);
        }
    });
}

function addAbbreviationAnswers(parsedAnswerline, answer, type) {
    ['getAbbreviation', 'extractUnderlining'].forEach(method => {
        const abbreviation = utils[method](answer);
        if (abbreviation.length > 1) {
            parsedAnswerline[type].push(abbreviation);
        }
    });
}

function addSpecialCaseAnswers(parsedAnswerline, answerline, type, ...regexes) {
    regexes.forEach(regex => {
        if (regex.test(answerline)) {
            parsedAnswerline.accept[0].split(' ').forEach(word => {
                if (type !== 'prompt') {
                    parsedAnswerline[type].push(word);
                } else {
                    parsedAnswerline[type].push([word, null]);
                }
            });
        }
    });
...
function processClause(clause, parsedAnswerline) {
    ...
    answers.forEach(answer => {
        if (directive !== 'prompt') {
            ...
            ['extractUnderlining', 'extractKeyWords', 'extractQuotes'].forEach(method => {
                parsedAnswerline[directive].push([utils[method](answer), directedPrompt]);
            });
        }
    });
}
...
function tokenListsMatch(given, reference, acceptSubstring, strictness, useStemmer) {
    for (const element of given) {
        if (!reference.some(ref => tokenMatches(element, ref, acceptSubstring, strictness, useStemmer))) {
            return false;
        }
    }
    return true;
...
function tokenMatches(element, reference, acceptSubstring, strictness, useStemmer) {
    const elemToCompare = useStemmer ? stemmer(element) : element;
    const refToCompare = useStemmer ? stemmer(reference) : reference;

    const errors = distance(elemToCompare, refToCompare);
    const isMatch = (strictness > 0 && strictness * errors <= reference.length) ||
        (acceptSubstring && reference.includes(element)) ||
        errors === 0;
...
function checkAnswer(answerline, givenAnswer) {
    ...
    if (shouldRejectImmediately(parsedAnswerline, givenAnswer) || checkAgainstAnswerList(parsedAnswerline.reject, givenAnswer, false)) {
        return { directive: 'reject', directedPrompt: null };
    } else if (checkAgainstAnswerList(parsedAnswerline.accept, givenAnswer, answerlineIsFormatted)) {
        return { directive: 'accept', directedPrompt: null };
    }

    return checkForPrompt(parsedAnswerline, givenAnswer, answerlineIsFormatted);
}

function shouldRejectImmediately(parsedAnswerline, givenAnswer) {
    return !parsedAnswerline.accept[0].includes('<u>') &&
           parsedAnswerline.accept[0].length > 1 &&
           givenAnswer.length === 1 && 
           isNaN(givenAnswer);
}

function checkAgainstAnswerList(answers, givenAnswer, answerlineIsFormatted) {
    for (const answer of answers) {
        const useStemmer = stemmer(answer) !== stemmer(answers[0]);
        if (answerlineIsFormatted) {
            if (stringMatchesReference({ string: answer, reference: givenAnswer, useStemmer })) {
                return true;
            }
        } else if (stringMatchesReference({ string: givenAnswer, reference: answer, acceptSubstring: true, useStemmer })) {
                return true;
        }
    }
    return false;
...
function checkForPrompt(parsedAnswerline, givenAnswer, answerlineIsFormatted) {
    for (const [answer, directedPrompt] of parsedAnswerline.prompt) {
        if (answerlineIsFormatted) {
            if (stringMatchesReference({ string: answer, reference: givenAnswer })) {
                return { directive: 'prompt', directedPrompt };
            }
        } else if (stringMatchesReference({ string: givenAnswer, reference: answer, acceptSubstring: true })) {
                return { directive: 'prompt', directedPrompt };
        }
    }
    return { directive: 'reject', directedPrompt: null };
