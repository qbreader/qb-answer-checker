...
const typoCorrections = loadCSVAsDict('typos.csv');
...
export default function tokenize(string, sort = false) {
    ...
    for (let i = 0; i <= tokens.length - 1; i++) {
        ...
        if (tokens[i] in honorificConversions) {
            tokens[i] = honorificConversions[tokens[i]];
        }

        if (tokens[i] in unitConversions) {
            tokens[i] = unitConversions[tokens[i]];
        }

        if (tokens[i] in britishConversions) {
        ...
        if (tokens[i] in typoCorrections) {
            tokens[i] = typoCorrections[tokens[i]][0];
        }
